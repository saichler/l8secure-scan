...
| Role | Scope | Notes |
|---|---|---|
| `customer` | One customer's data only | `L8User.associate_ids = [customerId]` (single-element list). Reuses the `AssociateIdsScopeView` pattern exactly as documented — no new placeholder is invented. Also `L8User.portal = "app.html?customerId=<customerId>"` (see note below) — the client-side source of the trusted `customerId` every write carries. |
| `opsadmin` | All customers | Internal Layer 8 support/ops role, allow-all rule, used to manage the `Customer` catalog itself and for support triage. Committed v1 scope (confirmed); UI-gated to the `System` area, not exposed to customer users. `portal` left empty for this role (falls back to `login.json`'s `redirectUrl`; never calls a customer-scoped write, so no `customerId` is needed). |

**Client-side `customerId` discovery — resolved in Phase 4, verified against real framework source, not assumed.** Every write in this app needs an explicit `customerId` in its request body (§9), but nothing in the login response gives the browser a way to know it: `l8api.AuthToken` (the real `/auth` response, `l8types/go/types/l8api/api.pb.go`) has exactly 7 fields — `token`/`error`/`setup_tfa`/`need_tfa`/`token_hash`/`portal`/`must_change_password` — no user id, no `associate_ids`, nothing tenant-shaped, confirmed by reading the Go handler that builds it (`l8web/go/web/server/WebService.go:160`, `l8secure/go/secure/provider/SecurityProvider.go:92-114`). No project in the ecosystem solves this for writes — even `l8erp`'s own reference customer/vendor/partner portal framework (`l8ui/portal/layer8d-portal.js`) only ever uses its scoping value (`scopeValue`, which it sets to the *login username*, not `associate_ids`) to build read-side `baseWhereClause` filters; it never stamps that value into a POST body anywhere. **Resolved by reusing `L8User.portal`** (the "HTTP path suffix for post-login redirect" field, `secure.proto`) as the `customerId` carrier instead: it is already threaded end-to-end by real, existing code with zero new endpoints — `TokenService.go`'s `newToken` sets `tk.Portal = usr.Portal`, the `/auth` response includes it (`AuthToken.Portal`), and the real login flow already persists it verbatim (`l8ui/login/layer8d-login-auth.js`: `sessionStorage.setItem('userPortal', result.portal)`). **Caveat verified and designed around, not glossed over**: `layer8d-login-auth.js`'s `getRedirectUrl(portalSuffix)` treats *any* truthy `portal` as the entire redirect path (`return '/' + suffix`), completely overriding `login.json`'s `redirectUrl` — so a bare `portal = "CUST-ACME"` would redirect the browser to the nonexistent path `/CUST-ACME` instead of `/app.html`, breaking login outright. Fixed by setting `portal` to `"app.html?customerId=<customerId>"` instead of the bare id: redirect still lands on the real page (with the id along for the ride as a query string), and the client reads the id back out of the *same* already-stored `sessionStorage.getItem('userPortal')` value by parsing it (`new URLSearchParams(portal.split('?')[1] || '').get('customerId')`) rather than using it raw — see `go/secscan/ui/web/secscan/secscan-session.js`. Every customer-role user's `L8User.portal` must be provisioned in this `"app.html?customerId=<id>"` form (Phase 6 mock data, and any future real provisioning) — never a bare id. **Mobile (Phase 5) needs its own equivalent** (`getRedirectUrl` prefixes `/m/` instead when `isMobileDevice()`), so the portal value's leading path segment isn't actually fixed across desktop/mobile — Phase 5 must re-verify this exact mechanism for `m/app.html`, not assume it carries over unchanged.
