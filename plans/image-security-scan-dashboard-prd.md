...
| Role | Scope | Notes |
|---|---|---|
| `customer` | One customer's data only | `L8User.associate_ids = [customerId]` (single-element list). Reuses the `AssociateIdsScopeView` pattern exactly as documented — no new placeholder is invented. Also `L8User.customer = <customerId>` (see note below) — the client-side source of the trusted `customerId` every write carries. |
| `opsadmin` | All customers | Internal Layer 8 support/ops role, allow-all rule, used to manage the `Customer` catalog itself and for support triage. Committed v1 scope (confirmed); UI-gated to the `System` area, not exposed to customer users. `customer` left empty for this role (never calls a customer-scoped write, so no `customerId` is needed). |

**Client-side `customerId` discovery — resolved at the framework level, not a project-local workaround.** Every write in this app needs an explicit `customerId` in its request body (§9), but the login response originally gave the browser no way to know it: `l8api.AuthToken` (the real `/auth` response) had exactly 7 fields — `token`/`error`/`setup_tfa`/`need_tfa`/`token_hash`/`portal`/`must_change_password` — no user id, no `associate_ids`, nothing tenant-shaped, confirmed by reading the Go handler that builds it (`l8web/go/web/server/WebService.go:160`, `l8secure/go/secure/provider/SecurityProvider.go:92-114`). No project in the ecosystem solved this for writes — even `l8erp`'s own reference customer/vendor/partner portal framework (`l8ui/portal/layer8d-portal.js`) only ever used its scoping value for read-side `baseWhereClause` filters, never a POST body. **Fixed properly, across three sibling repos, rather than worked around in this project**: `L8User` gained a real `customer` attribute (`l8secure` commit `2c2eb07`), threaded through `L8Token.customer` (`l8secure/proto/secure.proto`) → `authenticate()`/`SecurityProvider.Authenticate()` → `AuthToken.customer` (new field 8, `l8types` commit `0b4a316`) → `sessionStorage.setItem('userCustomer', result.customer)` in the real login flow (`l8ui/login/layer8d-login-auth.js`, commit `10d7fc7`) — the exact same path `portal` already used, so no new endpoints or client-side parsing tricks are needed. The client just reads `sessionStorage.getItem('userCustomer')` directly (`go/secscan/ui/web/secscan/secscan-session.js`). Every customer-role user must be provisioned with `L8User.customer` set to their own `customerId` (Phase 6 mock data, and any future real provisioning). **Live end-to-end as of this writing**: `l8types`/`l8secure`/`l8ui` have been pushed to their remotes, and this project's `l8ui` submodule bumped to `10d7fc7` to pick up the login-JS persistence commit. `l8secure/go` builds and vets clean with no replace/workaround needed (an earlier claim here about a `go.mod` staleness in `l8secure` was wrong — an artifact of testing against a not-yet-pushed local `l8types` checkout, retracted once corrected — see `plans/PROGRESS.md`). `go vet` did catch a real, since-fixed bug: 4 of 7 `authenticate()` call sites in `l8secure`'s own tests were missed when its return arity grew by one (`l8secure` commit `56596d3`).
...
- KPI strip: total image groups, total pending scans, total critical CVEs (customer-wide), groups with no scan yet. **Corrected in Phase 9**: not `Layer8DWidget.renderEnhancedStatsGrid` as originally written — read its source before using it and found it hardcoded to a different app's own navigation convention with a literal `value=0`, unusable here. Built from the lower-level `Layer8DWidget.render(kpi, value, opts)` instead, called once per real computed KPI number.
...
- `Layer8DTable` over `ImageRef`, `baseWhereClause: "imageGroupId='<id>'"`, default `sort-by buildDate desc` (requirement 5). Columns: checkbox (multi-select), Repo, Tag, Build Date (renders "Resolving…" while `buildDate=0` and no `scanError`, or an error indicator if resolution failed — §6.1 Phase B), Scan Status (`createStatusRenderer`), Total C/H/M/L, Distinct C/H/M/L. **Corrected in Phase 9**: `Layer8DTable` has **no native row-selection at all** (verified by reading its constructor directly — confirmed absent both on desktop and on mobile's `Layer8MTable`/`Layer8MEditTable`, not something this PRD's original "table's native row-selection" wording could rely on). Built from scratch instead: a `col.custom` checkbox column plus a `Set` tracked via event delegation on the table's container.
...
**`Layer8DModuleFilter` is not used — verified, and corrected here in Phase 9 (see `plans/PROGRESS.md`).** `Layer8DModuleFilter` (`await Layer8DModuleFilter.load(bearerToken)` / `.applyToSidebar()`) is the **global, ModConfig-backed** module enable/disable mechanism (fetches `/0/ModConfig`) — this project has no `ModConfig` service, so per `ModconfigFailureNoLogout` that block is deliberately **not** wired into `app.js` (its 404 would otherwise trigger an internal `logout()` and an infinite redirect loop).

This section originally also specified project-specific role-based nav hiding ("after login, the app reads the current user's role from the auth response and conditionally omits the `admin` entry") — **never implemented, because it isn't possible with this framework's real `/auth` response**: verified in Phase 4 that neither the `/auth` JSON body nor the bearer JWT (`l8secure`'s `Claim` type is a bare `jwt.StandardClaims` — no roles field) carries the caller's role to the client, and no verified endpoint exposes "my own roles" either. The `admin` nav link is therefore visible to every logged-in user regardless of role (both desktop and mobile) — this is documented in `js/app.js` at the point where it would otherwise have been wired in. This is **not a security gap**: the actual boundary is the server-side deny-before-allow permission check (§14) — a `customer`-role user's request against `Customer` is rejected by `ISecurityProvider` regardless of what the nav shows, since the `customer` role's rules grant no `Customer` permissions at all; a `customer`-role user who clicks into Administration just sees an empty/erroring table.

### 11.7 Mobile

**Rewritten in Phase 9 to match what Phase 5 actually verified and built** — the paragraph originally here described the pre-implementation design intent, not the real `l8ui` mobile API (see `plans/PROGRESS.md`'s Phase 5 entry and deviations for the full verification trail).

Full parity per `MobileRules`, via two nav-config modules (`vulnmgmt`, `admin`) each with exactly one `subModules` entry (auto-skips straight to the services list) plus a `system` module (Health Monitor + the shared `L8Security` Users/Roles/Credentials columns/forms desktop's own System section already uses). `Layer8MModuleRegistry.create('MobileSecScan', {...})` combines `MobileSecScanVuln`/`MobileSecScanAdmin`, built by decorating desktop's own already-loaded enum/column/form files with `primary`/`secondary`/`hidden` card flags (`.map()`+`Object.assign` — the real, documented mobile convention; there is no separate mobile column factory). Categories/Scan History/Customers use the generic `Layer8MNavData`/`Layer8MEditTable` CRUD pipeline. Image Groups instead uses the real `customInit` service-view mechanism (not `Layer8MEditTable`), since the generic pipeline has no extension point for the KPI strip + Add Images/Export CSV Report toolbar this list needs. Group Detail and Vulnerability Detail are each `Layer8MPopup` + a hand-built `Layer8MTable` with `baseWhereClause` (not `Layer8MNav.navigateToService`, which has no real per-parent-scoping mechanism and no real onCardClick precedent) — Group Detail's multi-select is a hand-built checkbox column, since `Layer8MTable`/`Layer8MEditTable` have no native row-selection (verified, matches desktop's own confirmed gap). **No mobile severity chart** — `Layer8MChart` has zero real usage anywhere in the ecosystem, and a `Layer8DChart`-on-mobile composition considered as a replacement could not be re-verified in the time available; flagged as a disclosed, deliberate scope gap rather than shipping an unverified composition. CSV export reuses the same `VulnRep` endpoint, POSTed via `Layer8MAuth.post`, with the exact same Blob+`<a download>` trick as desktop (verbatim, no mobile-specific download handling exists anywhere in the ecosystem).
...
| Module abstractions (service-registry, module-crud, module-navigation, toggle-tree, module-filter, module-factory-core, module-factory) | Yes (script loaded per canonical order) | Required, **except** `Layer8DModuleFilter.load()`/`.applyToSidebar()` is deliberately never *called* from `app.js` — this project has no `ModConfig` service, and invoking it would hit `ModconfigFailureNoLogout`'s 404→logout loop. There is no `admin`-module visibility gating at all (corrected in Phase 9 — see §11.6): the real `/auth` response carries no role to gate on, so the `admin` link is visible to every logged-in user; the actual boundary is the server-side deny-before-allow check (§14) |
...
**Corrected in Phase 9 against what Phase 5 actually shipped** (`go/secscan/ui/web/m/app.html`): core mobile CSS/JS, popup/confirm/table/edit-table/forms/shared reference-registry all **Yes**; `layer8m-datepicker.js`/CSS **Yes** (loaded defensively, matches the shared component set — no editable date field in this app actually exercises it, same as desktop); `layer8m-chart.js` **N/A, not `Yes` as originally written here** — has zero real usage anywhere in the ecosystem (verified), and no mobile severity chart was built at all (a disclosed scope gap, see §11.7); `layer8m-kanban/calendar/timeline/gantt/tree-grid/wizard.js` **N/A** for the same domain reasons as desktop; AI agent mobile files **N/A**. Project-specific: `nav-configs/layer8m-nav-config-secscan.js` (nav core loads before this, per `ScriptLoadingOrder`) and `js/secscan/**` (module data/behavior files, §11.7). **No project-specific mobile reference-registry file exists, unlike originally written here** — the shared `layer8m-reference-registry.js` is loaded but nothing registers a project entry into it: the one place a category needs picking (Group Detail's header) calls `Layer8MReferencePicker.show()` directly with an inline endpoint/modelName config, not through a registered lookup, and `ImageGroup`'s own reused form (which does have an `f.reference('categoryId', ...)` field) is never actually rendered in mobile practice — `customInit` bypasses the generic form pipeline for Image Groups entirely, matching desktop's own equivalent fallback-form situation.
...
2. Parse Trivy's JSON output:
   ...
   - **Re-scan cleanup (gap in an earlier draft, fixed in Phase 3 — see `plans/PROGRESS.md`):** once Trivy succeeds and its output parses, delete every existing `ImageRefCve` row for this `imageRefId` (`select * from ImageRefCve where imageRefId='<id>'` via a `DELETE`) *before* writing the new findings below. Without this, re-scanning an already-scanned `ImageRef` (a normal, expected action, not an edge case) would just add the new scan's findings on top of the previous scan's, so §11.4's findings list would silently accumulate stale/duplicate rows forever instead of reflecting only the latest scan. Done only after Trivy succeeds (not before invoking it), so a failed scan attempt leaves the previous scan's findings intact rather than wiping them for nothing.
...
**Verified in Phase 3 (see `plans/PROGRESS.md`): `scanner`/`scanner-svc` are not actually consulted by `secscan-scanner`'s own vnic calls to the backend, and that's fine, not a bug.** `secscan-scanner` never logs in or attaches any credential when it calls `vnic.Request`/`common.GetEntity`/`PutEntity`/`PostEntity` (confirmed against real precedent — `../probler`'s `collector` binary, a directly analogous separate-process, non-ORM worker, does the same with zero login step). Traced the actual call chain (`l8bus/go/overlay/protocol/Protocol.go`, `l8secure/go/secure/provider/SecurityProvider.go`, `l8secure/go/secure/provider/ScopeView.go`): a raw vnic call with no explicit token gets `Security().Message(...)` falling back to the process's own `SysConfig().LocalAlias` as its `aaaid`, which is always shorter than the 36-character validated-token length `CanDoAction`/`ScopeView` require — both have an explicit "internal/system calls bypass scoping" short-circuit for exactly this case. Only `l8web`'s HTTP bridge ever produces a real, validated `aaaid` from a bearer token. So this project's actual security boundary for `secscan-scanner`'s traffic is network trust (which pods can reach the vnet), not per-call AAA — the `scanner`/`scanner-svc` role and user still describe the *intended* identity/permission model accurately (and cost nothing to keep, matching every other identity in this config), but nothing in the framework currently enforces them against `secscan-scanner`'s own calls. This mirrors the same class of framework limitation already accepted for `customer_id` trust (§4/§9) — not something to "fix" by inventing a login flow that has no real precedent anywhere in the ecosystem.

`login.json` (`LoginJsonAdaptation`): `apiPrefix` = `/scan` (not `/erp`); `appTitle` = "Layer 8 Secure Scan".

## 15. Mock Data (`go/tests/mocks/`)

**Rewritten in Phase 6 per explicit user instruction (2026-09-13): "you don't need mock data, just use the images of this project and customer name local."** The phased/fabricated design originally specified here (3 customers, a synthetic `Cve` catalog, simulated `ImageRefCve` scan results, historical `ScanJob` records — see `plans/PROGRESS.md`'s Phase 6 entry for the full original text and reasoning) was replaced with a much smaller, real-data seed:

1. **One customer** — `Customer{customerId:"local"}`.
2. **Categories** — 2 categories for `local` via `ImgCat` POSTs.
3. **This project's own real container images** — the 6 binaries + 2 base images from §16's own table (`saichler/<name>:latest`), registered via the real `ImgRefAdd` bulk-ingestion action service (§6.1) — not a direct `ImageGroup`/`ImageRef` POST, and not fabricated repo names. Every seeded `ImageRef` starts `PENDING`, exactly as it would for a real user adding real images.
4. **No `Cve` catalog, no `ImageRefCve`/scan-result fabrication, no historical `ScanJob` records at all.** A real `secscan-scanner` run against these images (once §16's images are actually built) produces real Trivy findings — there is nothing genuine to simulate ahead of that.
5. **Security users** — one `customer`-role user for `local` (`associateIds:["local"]`, `customer:"local"`) plus the already-seeded `opsadmin`/`scanner-svc` users (§14), provisioned via the Security API (`/73/users`), never a project-owned endpoint (`SecurityRules`).

Implemented as a single small `go/tests/mocks/seed.go` (plus the unchanged `client.go`/`main.go` HTTP-client scaffolding) rather than the full phased `data.go`/`store.go`/`gen_*.go`/`phases.go` machinery `MockDataRules` describes — that structure exists for large randomized datasets with cross-module FK dependencies; ~10 fixed literal records have no such need.
...
- [x] Mock Data: scope reduced per explicit user instruction (§15, 2026-09-13) — one real customer ("local") seeded with this project's own real images via the real `ImgRefAdd` path, no fabricated CVE/scan data; Security-API user provisioning (not project-owned) unchanged.
- [x] Deployment: `build.sh` + `Dockerfile` per binary, K8s YAMLs for all 4 modes + KIND scripts, `run-local.sh` (§16–§17).
- [x] Configuration: `login.json` adapted (`apiPrefix=/scan`), no `ModConfig` dependency — the l8erp `Layer8DModuleFilter.load()` block is deliberately not called from copied `app.js` (§11.6, §12.2), per `ModconfigFailureNoLogout`. **Corrected in Phase 9**: the `admin` module has no role-based nav hiding at all (not "project-specific role logic instead" as originally written here) — verified the real `/auth` response carries no role information to hide it with; the `admin` link is visible to every logged-in user, and the real security boundary is the server-side deny-before-allow check (§11.6, §14), not the nav.
